...
if __name__ == "__main__":
    ...
    sheet_SP = wb.sheet_by_name('SP_Customers')
    sheet_inv = wb.sheet_by_name("Invoices_Master")

    jimsSP = {}
    for row in range(1, sheet_SP.nrows):
        account = str(sheet_SP.cell(row,0)).replace("'", "").replace("number:", "")
        account = str(int(float(account)))
        account = "0"*(7 - len(account)) + account

        company = str(sheet_SP.cell(row,1)).replace("'", "").replace("text:", "").replace('"', "")
    ...
    # print("---------------------------------")
    # print("Populating Customer's info")
    # print("----------------------------------")
    ...
    for row in range(1, sheet_inv.nrows):
        ...
        if account in jimsSP:
            ...
            jimsSP[account][str(year)][str(month)]  += round((nontax + taxable), 2)

    group = CompanyGroup.objects.get(name="SpecailPricingCustomers")

    for acc in jimsSP:

        company = Company.objects.get_or_create(
            name = jimsSP[acc]['company'],
            account = jimsSP[acc]['account'],
        )[0]
        ...
        # 2014 -----------------------------------
        for mst in jimsSP[acc]['2014'].items():
        ...
        # 2015 -----------------------------------
        for mst in jimsSP[acc]['2015'].items():
        ...
        # 2016 -----------------------------------
        for mst in jimsSP[acc]['2016'].items():
        ...
        # 2017 -----------------------------------
        for mst in jimsSP[acc]['2017'].items():
            mst_object = MonthlySalesTotal.objects.get_or_create(
                company = company,
                month = mst[0],
                year = 2017,
        ...
        # 2018 -----------------------------------
        for mst in jimsSP[acc]['2018'].items():
            mst_object = MonthlySalesTotal.objects.get_or_create(
                company = company,
                month = mst[0],
                year = 2018,
        ...
        # 2019 -----------------------------------
        for mst in jimsSP[acc]['2019'].items():
            mst_object = MonthlySalesTotal.objects.get_or_create(
                company = company,
                month = mst[0],
                year = 2019,
        ...
        # 2020 -----------------------------------
        for mst in jimsSP[acc]['2020'].items():
            mst_object = MonthlySalesTotal.objects.get_or_create(
                company = company,
                month = mst[0],
                year = 2020,
